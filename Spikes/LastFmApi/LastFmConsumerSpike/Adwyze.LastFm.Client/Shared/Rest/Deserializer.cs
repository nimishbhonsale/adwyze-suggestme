    ...
    public class Deserializer : IDeserializer
	{
        ...
        private void Map(object x, XElement root)
		{
			...
			foreach (var prop in props)
			{
				...
				{
				    value = DateFormat.HasValue() ? DateTime.ParseExact(value.ToString(), DateFormat, Culture) : DateTime.Parse(value.ToString(), Culture);
				    prop.SetValue(x, value, null);
				}
				...
				else if (type.IsGenericType)
				{
					...
					var elements = container.Elements().Where(d => first != null && d.Name == first.Name);
				...
				else if (type.IsSubclassOfRawGeneric(typeof(List<>)))
				{
					...
					var list = HandleListDerivative(root, name.ToString(), type);
					prop.SetValue(x, list, null);
				}
				else
				{
